...
class DeliveryManController extends Controller
{
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $delivery_men=DeliveryMan::query()->where(function ($q) use ($key) {
    ...
    public function preview(Request $request, $id, $tab='info')
    {
        ...
        if($tab == 'info')
        {
            $reviews=DMReview::query()->where(['delivery_man_id'=>$id])->latest()->paginate(config('default_pagination'));
        ...
        {
            $user = UserInfo::query()->where(['deliveryman_id' => $id])->first();
            if($user){
    ...
    {
        $delivery_man = DeliveryMan::query()->find($request->id);
            ...
            if ($request->status == 0) {
                ...
                if (isset($delivery_man->fcm_token)) {
                    $data = [
                        ...
                        'description' => translate('messages.your_account_has_been_approved'),
                        'order_id' => '',
                        'image' => '',
                        'type' => 'block'
                    ];
