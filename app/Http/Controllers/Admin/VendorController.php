...
class VendorController extends Controller
{
    ...
    public function update_setup_delivery_radius(Store $store, Request $request){
        $request->validate([
            'delivery_radius' => 'required',
        ]);
        $store->delivery_radius = to_meters($request->delivery_radius);
        $store->save();
    ...
    public function consolidate_cycle($id, Request $request){
        ...
        if(isset($store) && isset($wallet)){
            $invoice = new Invoices();
            $invoice->store_id = $store->id;
            $invoice->zone_id = $store->zone_id;
            $invoice->module_id = $store->module_id;
            $invoice->month_year = now()->setTimeFromTimeString($wallet->current_cycle)->format('m-Y');
            $invoice->initial_date = now()->setTimeFromTimeString($wallet->current_cycle);
            $invoice->final_date = now();
            $invoice->total_gain = $wallet->total_earning;
            $invoice->money_in_hand = $wallet->money_in_hands;
            $invoice->invoice_balance = invoice_balance($wallet);
            $invoice->due_date = now()->setTimeFromTimeString($request->date_due);
            $invoice->type = invoice_balance_type($wallet);
            $invoice->consolidation_date = now();
            ...
            $wallet->updated_at=now();
            $wallet->save();
            Toastr::success(translate('messages.new_Invoice_and_new_cicly'));
            return back();
        }
    ...
    public function view(Store $store, Request $request, $tab=null, $sub_tab='cash')
    {
        $wallet = $store->vendor->wallet;
        if(!$wallet)
        {
        ...
        else if($tab == 'reviews')
        {
            ...
            return view('admin-views.vendor.view.wallet', compact('store', 'invoices', 'wallet', 'total_gain', 'money_in_hand', 'total_paid', 'credit_balance', 'debit_balance'));
        }else if($tab == 'invoices'){
            if(session()->has('filter_invoices_store'))
            {
                foreach (json_decode(session('filter_invoices_store')) as $key => $value){
                    $request[$key] = $value;
                }
            }
            ...
            return view('admin-views.vendor.invoices.list', compact('invoices', 'store', 'month_year', 'invoices_status', 'status', 'from_date', 'to_date', 'types'));
    ...
    public function updateStoreSettings(Store $store, Request $request)
    {
        $request->validate([
            ...
            'comission'=>'required',
            'cashback_store'=>'required',
            'tax'=>'required',
            'minimum_delivery_time' => 'required|regex:/^([0-9]{2})$/|min:2|max:2',
            'maximum_delivery_time' => 'required|regex:/^([0-9]{2})$/|min:2|max:2',
        ]);

        if($request->no_commission_default)
        {
            $store->comission = $request->comission;
            $store->no_commission_default = true;
        }
        else{
            $store->comission = 0.00;
            $store->no_commission_default = false;
        }
        if($request->no_cashback_default)
        {
            $store->cashback_store = $request->cashback_store;
            $store->no_cashback_default = true;
        }
        else{
            $store->cashback_store = 0.00;
            $store->no_cashback_default = false;
        }
    ...
    public function filter_invoices_store(Request $request): RedirectResponse
    {
        $request->validate([
            'from_date' => 'required_if:to_date,true',
            'to_date' => 'required_if:from_date,true',
        ]);
        session()->put('filter_invoices_store', json_encode($request->all()));
        return back();
    }

    public function filter_reset_invoices_store(): RedirectResponse
    {
        session()->forget('filter_invoices_store');
        return back();
    }
    ...
    public function filter_invoices(Request $request): RedirectResponse
    {
        ...
        session()->put('filter_invoices', json_encode($request->all()));
        return back();
    }

    public function filter_reset_invoices(): RedirectResponse
    {
        session()->forget('filter_invoices');
        return back();
    }

    public function list_invoices(Request $request){
        if(session()->has('filter_invoices'))
        {
            foreach (json_decode(session('filter_invoices')) as $key => $value){
                $request[$key] = $value;
            }
        }
        $invoices_status = Invoices::query()
        ...
        $invoices = Invoices::with('store')->with('module')->with('zone')
            ->orderBy('created_at' , 'desc')
            ->when(isset($request->module_id) && $request->module_id != 'null', function ($query)use($request){
                return $query->where('module_id', $request->module_id);
            })
            ->when(isset($request->zone_id) && $request->zone_id != 'null', function ($query)use($request){
            ...
            ->when(isset($request->store_id) && $request->store_id != 'null', function ($query)use($request){
        ...
        if(isset($request->module_id) && $request->module_id != 'null'){
            $module_id = $request->module_id;
        }
        $month_year = $request->month_year;
//        dd($from_date, $to_date, $status, $types, $store_id, $zone_id, $module_id, $month_year);

        return view('admin-views.invoices.list', compact('invoices', 'month_year', 'invoices_status', 'status', 'from_date', 'to_date', 'types', 'store_id', 'zone_id', 'module_id'));
    }
    ...
    public function print_invoice($id){
        ...
        if(isset($invoice)){
            return view('admin-views.invoices.invoice', compact('invoice'));
        }
        Toastr::error('Fatura não encontrada');
        return back();
    }
    ...
    public function print_invoice_print($id){
        $invoice = Invoices::with('store')->find($id);
        if(isset($invoice)){
            return view('admin-views.invoices.invoice-print', compact('invoice'))->render();
        }
        Toastr::error('Fatura não encontrada');
        return back();
    }

    public function date_due(Request $request){
        $invoice = Invoices::query()->find($request->id_invoice);
        if(isset($invoice) && isset($request->date_due)){
             $invoice->due_date = $request->date('date_due');
            $invoice->status = 'pending';
            $invoice->save();
    ...
    public function deny_payment($id): RedirectResponse
    {
        $invoice = Invoices::with('store')->find($id);
        if(isset($invoice)){
    ...
    public function confirm_payment($id): RedirectResponse
    {
        $invoice = Invoices::with('store')->find($id);
        if(isset($invoice)){
