...
class ReportController extends Controller
{
    ...
    public function day_wise_report(Request $request)
    {
        ...
        $order_transactions = OrderTransaction::with('translationOrder')->when(isset($zone), function ($query) use ($zone) {
        ...
        $transactions = WalletTransaction::query()
            ...
            ->selectRaw(DB::raw("sum(`credit`) as total_credit, count(*) as credit, IF((`transaction_type`='cashback'), `transaction_type`, IF(`transaction_type`='reference',`transaction_type`, IF(`transaction_type`='affiliate',`transaction_type`, `transaction_type`))) as 'transaction_types'"))->groupBy('transaction_types')->get();
        $totalSum = 0;
        foreach ($transactions as $transaction){
            $totalSum += $transaction->total_credit;
        }
        return view('admin-views.report.day-wise-report', compact('order_transactions', 'zone', 'totalSum', 'transactions'));
    }
...
//            ->selectRaw(DB::raw("sum(`order_amount`) as total_order_amount, count(*) as order_count, IF((`payment_method`='cash_on_delivery'), `payment_method`, IF(`payment_method`='wallet',`payment_method`, 'payment_pix_offline')) as 'payment_methods'"))->groupBy('payment_methods')
            ->selectRaw(DB::raw("sum(`order_amount`) as total_order_amount, count(*) as order_count, IF((`payment_method`='cash_on_delivery'), `payment_method`, IF(`payment_method`='wallet',`payment_method`, IF(`payment_method`='payment_credit_card_offline',`payment_method`, IF(`payment_method`='payment_debit_card_offline',`payment_method`, IF(`payment_method`='payment_pix_offline',`payment_method`, 'online_payment'))))) as 'payment_methods'"))->groupBy('payment_methods')
            ->get();
    ...
    public function store_order_report(Request $request)
    {
        ...
        $order_payment_methods = Order::when(isset($zone), function ($query) use ($zone) {
            ...
            ->StoreOrder()->NotRefunded()
            ->selectRaw(DB::raw("sum(`order_amount`) as total_order_amount, count(*) as order_count, IF((`payment_method`='cash_on_delivery'), `payment_method`, IF(`payment_method`='wallet',`payment_method`, IF(`payment_method`='payment_pix_offline',`payment_method`, IF(`payment_method`='payment_debit_card_offline',`payment_method`, IF(`payment_method`='payment_credit_card_offline',`payment_method`, 'online_payment'))))) as 'payment_methods'"))
