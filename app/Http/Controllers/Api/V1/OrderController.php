...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cash_on_delivery,payment_credit_card_offline,payment_debit_card_offline,payment_pix_offline,wallet',
        ...
        if ($request->order_type !== 'parcel') {
            ...
            $store = Store::with('discount')
//                ->whereRaw('ST_Distance_Sphere(point(longitude, latitude),point('.$request->longitude.', '.$request->latitude.')) as distance')
                ->selectRaw('*, IF(((select count(*) from `store_schedule` where `stores`.`id` = `store_schedule`.`store_id` and `store_schedule`.`day` = ' . $schedule_at->format('w') . ' and `store_schedule`.`opening_time` < "' . $schedule_at->format('H:i:s') . '" and `store_schedule`.`closing_time` >"' . $schedule_at->format('H:i:s') . '") > 0), true, false) as open')->where('id', $request->store_id)->first();
        ...
        if ($request->latitude && $request->longitude) {
            ...
            if (!$zone) {
                $errors = [];
                $errors[] = ['code' => 'coordinates', 'message' => translate('messages.out_of_coverage')];
                return response()->json([
                    'errors' => $errors
                ], 403);
            }
        }
        if($store->is_delivery_radius && $request['order_type'] == 'delivery'){
            if($request->distance > $store->delivery_radius){
                return response()->json([
                    'message' => trans('messages.this_shop_does_not_deliver_to_your_address')
                ], 403);
            }
        }
        ...
        $order_details = [];
        $order = new Order();
        $order->id = id_order_generator();
        if (Order::find($order->id)) {
            $order->id = id_order_generator() + 1;
        }
        $order->user_id = $request->user()->id;
        $order->thing = $request['thing'] ?? '';
        $order->order_amount = $request['order_amount'];
        $order->payment_status = $request['payment_method']=='wallet'?'paid':'unpaid';
        $order->order_status = $request['payment_method'] == 'wallet'?'confirmed':'pending';
        ...
        $order->original_delivery_charge = round(($original_delivery_charge??0), config('round_up_to_digit'));
        ...
        $cashback = $store->no_cashback_default ? $store->cashback_store : \App\Models\BusinessSetting::where('key','cashback_default')->first()->value;
        $cashback_generated = ($request['order_amount'] - $delivery_charge - $order->total_tax_amount )* ($cashback/100);
        $order->cashback_generated = $cashback_generated;
        ...
        try {
            ...
            $customer->save();
            if($request->payment_method == 'wallet') CustomerLogic::create_wallet_transaction($order->user_id, $order->order_amount, 'order_place', $order->id, $store->name);
            DB::commit();
