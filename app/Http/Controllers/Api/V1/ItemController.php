...
class ItemController extends Controller
{
    public function get_latest_products(Request $request)
    {
        ...
        $items = ProductLogic::get_latest_products($zone_id, $request['limit'], $request['offset'], $request['store_id'], $request['category_id'], $type, $longitude, $latitude);
    ...
    public function get_popular_products(Request $request)
    {
        ...
        $zone_id= $request->header('zoneId');
        $items = ProductLogic::popular_products($zone_id, $longitude, $latitude, $request['limit'], $request['offset'], $type);
        $items['products'] = Helpers::product_data_formatting($items['products'], true, false, app()->getLocale());
        return response()->json($items, 200);
    }
    ...
    public function get_most_reviewed_products(Request $request)
    {
        ...
        $latitude= $request->header('latitude');
        $zone_id= $request->header('zoneId');
        $items = ProductLogic::most_reviewed_products($zone_id, $longitude, $latitude, $request['limit'], $request['offset'], $type);
        $items['products'] = Helpers::product_data_formatting($items['products'], true, false, app()->getLocale());
        return response()->json($items, 200);
    }
    ...
    public function get_related_products(Request $request,$id)
    {
        ...
        $latitude= $request->header('latitude');
        if (Item::find($id)) {
            $items = ProductLogic::get_related_products($zone_id,$id, $longitude, $latitude);
