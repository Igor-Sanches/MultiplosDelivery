...
class CategoryController extends Controller
{
    public function get_subcategories(Request $request){
        try {
            $categories = Category::query()->where(['position'=>1,'status'=>1])
                ->when(config('module.current_module_data'), function($query){
                    $query->module(config('module.current_module_data')['id']);
                })
                ->selectRaw('*, IF((select count(*) from `items` where `categories`.`id` = `items`.`category_id`) > 0, true, false) as with_items')
                ->orderBy('priority','desc')->get();
            return response()->json(Helpers::category_data_formatting($categories, true));
        } catch (\Exception $e) {
