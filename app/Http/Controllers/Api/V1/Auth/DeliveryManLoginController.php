...
class DeliveryManLoginController extends Controller
{
    public function login(Request $request): JsonResponse
    {
        ...
        if (auth('delivery_men')->attempt($data)) {
            ...
            {
                $errors = [];
                $errors[] = ['code' => 'auth-003', 'message' => translate('messages.your_account_has_been_suspended')];
                return response()->json([
                    'errors' => $errors
                ], 401);
            }

            $delivery_man =  DeliveryMan::query()->where(['phone' => $request['phone']])->first();
            ...
            $errors = [];
            $errors[] = ['code' => 'auth-001', 'message' => 'Não autorizado.'];
            return response()->json([
                'errors' => $errors
            ], 401);
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $dm->status = 0;
        $dm->active = 0;
//        $dm->fcm_token = $request->fcm_token ?? '';
        ...
        return response()->json(['message' => translate('messages.deliveryman_added_successfully'), 'token' => $token, 'zone_wise_topic'=>$dm->type=='zone_wise'?$dm->zone->deliveryman_wise_topic:'restaurant_dm_'.$dm->store_id], 200);
    }
}
