...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            ...
            'f_name.required' => 'O campo do primeiro nome é obrigatório.',
            'l_name.required' => 'O campo sobrenome é obrigatório.',
        ]);
...
//        $customer_verification = BusinessSetting::where('key', 'customer_verification')->first()->value;
        ...
        $check_duplicate_ref = WalletTransaction::query()->where('reference', $request->phone)->first();
        ...
        if ($check_duplicate_ref) {
            ...
            if ($request->ref_code){
                ...
                $referar_user = User::query()->where('ref_code', '=', $checkRefCode)->first();
                if (!isset($referar_user)) {
                    $errors = [];
                    $errors[] = ['code' => 'ref_code', 'message' => translate('messages.referer_code_not_found')];
                    return response()->json([
                        'errors' => $errors
                    ], 405);
                }
                $user->registered_with_the_code = true;
                $user->affiliate = $checkRefCode;
                $user->save();
                try{
                    $fcm_token = $referar_user->cm_firebase_token;
                    $data = [
                        'title' =>($user->f_name.' '.$user->l_name).' '.translate('messages.registered_with_you_code'),
                        'description' => translate('messages.now_are_winning_together'),
                        'order_id' => '',
                        'image' => '',
                        'message' => json_encode(''),
                        'type'=> 'affiliate',
            ...
            return response()->json(['token' => $token, 'is_phone_verified' => 1, 'phone_verify_end_url' => "api/v1/auth/verify-phone"]);
        }
