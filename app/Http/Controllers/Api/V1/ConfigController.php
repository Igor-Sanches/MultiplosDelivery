...
class ConfigController extends Controller
{
    ...
    public function configuration()
    {
        ...
        $payment_credit_card_offline = json_decode($settings['payment_credit_card_offline'], true);
        $payment_debit_card_offline = json_decode($settings['payment_debit_card_offline'], true);
        $payment_pix_offline = json_decode($settings['payment_pix_offline'], true);
...
//        }
        return response()->json([
            ...
            'cash_on_delivery' => (boolean)$cod['status'] == 1,
            'payment_credit_card_offline' => (boolean)$payment_credit_card_offline['status'] == 1,
            'payment_debit_card_offline' => (boolean)$payment_debit_card_offline['status'] == 1,
            'payment_pix_offline' => (boolean)$payment_pix_offline['status'] == 1,
            ...
            'demo'=>(boolean)env('APP_MODE')=='demo',
    ...
    public function get_zone(Request $request)
    {
        ...
        $zones = Zone::with('modules')->contains('coordinates', $point)->latest()->get(['id', 'status', 'cash_on_delivery', 'payment_pix_offline', 'payment_credit_card_offline', 'payment_debit_card_offline']);
        if(count($zones)<1)
        {
            return response()->json([
                'errors'=>[
