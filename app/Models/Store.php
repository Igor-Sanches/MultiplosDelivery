...
class Store extends Model
{
    ...
    protected $casts = [
        ...
        'payment_credit_card_offline'=>'boolean',
        'payment_debit_card_offline'=>'boolean',
        'payment_pix_offline'=>'boolean',
        'free_delivery'=>'boolean'
    ];

    protected $appends = ['gst_status','gst_code', 'free_delivery_2', 'cashback'];

    public function getCashbackAttribute(){
        $is_cash_back_enabled = $this->no_cashback_default;
        if($is_cash_back_enabled){
            return $this->cashback_store;
        }else{
            return BusinessSetting::query()->where('key', 'cashback_default')->first()->value;
        }
    }

    public function getFreeDelivery2Attribute(){
     ...
     */
    protected $hidden = [
        'gst', 'no_commission_default', 'no_cashback_default', 'is_delivery_radius', 'delivery_radius', 'cashback_store', 'comission'
    ];
    ...
    public function scopeWithDistanceRadius($query, $longitude,$latitude){
        return $query->selectRaw('IF(is_delivery_radius = 0, true, IF(delivery = 0, false, IF(ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) <= delivery_radius, true, false))) as delivery_distance');
//        return $query->whereRaw('IF(is_delivery_radius = 0, true, IF(ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) <= delivery_radius, true, false))');
    }
