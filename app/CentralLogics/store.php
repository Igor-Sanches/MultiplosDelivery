...
class StoreLogic
{
    public static function get_stores( $zone_id, $filter, $type, $limit = 10, $offset = 1, $featured=false,$longitude=0,$latitude=0)
    {
        $paginator = Store::query()
            ...
            ->withDistanceRadius($longitude, $latitude)
    ...
    {
        $is_delivery_radius = BusinessSetting::where(['key' => 'is_delivery_radius'])->first()->value;
        $paginator = Store::query()
            ->withOpen($longitude,$latitude)
            ->withDistanceRadius($longitude, $latitude)
            ->with(['discount'=>function($q){
    ...
    {
        $is_delivery_radius = BusinessSetting::where(['key' => 'is_delivery_radius'])->first()->value;
        $paginator = Store::query()->withOpen($longitude,$latitude)
            ->withDistanceRadius($longitude, $latitude)
            ->with(['discount'=>function($q){
            return $q->validate();
        }])
    ...
    public static function get_store_details($store_id,$longitude,$latitude)
    {
        return Store::query()
            ->selectRaw('*, IF(is_delivery_radius = 0, true, IF(delivery = 0, false, IF(ST_Distance_Sphere(point(longitude, latitude),point('.$longitude.', '.$latitude.')) <= delivery_radius, true, false))) as delivery_distance')
            ->with(['discount'=>function($q){
            return $q->validate();
        }, 'campaigns', 'schedules'])
    ...
    {
        $key = explode(' ', $name);
        $paginator = Store::query()->whereHas('zone.modules', function($query){
            $query->where('modules.id', config('module.current_module_data')['id']);
        })->withOpen($longitude,$latitude)
            ->withDistanceRadius($longitude, $latitude)
            ->with(['discount'=>function($q){
