...
class Helpers
{
    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en')
    {
        ...
        if ($multi_data == true) {
           foreach ($data as $item) {
               ...
               $is_cash_back_enabled = $item->store->no_cashback_default;
               if($is_cash_back_enabled){
                   $cashback = $item->store->cashback_store;
               }else{
                   $cashback = BusinessSetting::query()->where('key', 'cashback_default')->first()->value;
               }
            ...
            unset($data['store']);
            unset($data['rating']);
            $is_cash_back_enabled = $data->store->no_cashback_default;
            if($is_cash_back_enabled){
                $cashback = $data->store->cashback_store;
            }else{
                $cashback = BusinessSetting::query()->where('key', 'cashback_default')->first()->value;
            }
    ...
    public static function store_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $is_cash_back_enabled = $item->no_cashback_default;
                if($is_cash_back_enabled){
                    $cashback = $item->cashback_store;
                }else{
                    $cashback = BusinessSetting::query()->where('key', 'cashback_default')->first()->value;
                }
                $item['cashback'] = $cashback;
            ...
            $is_cash_back_enabled = $data->no_cashback_default;
            if($is_cash_back_enabled){
                $cashback = $data->cashback_store;
            }else{
                $cashback = BusinessSetting::query()->where('key', 'cashback_default')->first()->value;
            }
            $item['cashback'] = $cashback;
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $status == 'pending' && has_payment_offline($order->payment_method) && config('order_confirmation_model') == 'deliveryman') {
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $order->order_status == 'pending' && has_payment_offline($order->payment_method) && config('order_confirmation_model') == 'store') {
                $data = [
                    'title' => translate('messages.order_push_title'),
                    'description' => translate('messages.new_order_push_description'),
                    'order_id' => $order->id,
            ...
            if (!$order->scheduled && (($order->order_type == 'take_away' && $order->order_status == 'pending') || (!has_payment_offline($order->payment_method) && $order->order_status == 'confirmed'))) {
                $data = [
                    'title' => translate('messages.order_push_title'),
                    'description' => translate('messages.new_order_push_description'),
                    'order_id' => $order->id,
            ...
            if ($order->order_status == 'confirmed' && $order->order_type != 'take_away' && config('order_confirmation_model') == 'deliveryman' && has_payment_offline($order->payment_method)) {
                if ($order->store->self_delivery_system) {
                    $data = [
                        'title' => translate('messages.order_push_title'),
                        'description' => translate('messages.new_order_push_description'),
            ...
            if ($order->order_type == 'delivery' && !$order->scheduled && $order->order_status == 'confirmed'  && (!has_payment_offline($order->payment_method) || config('order_confirmation_model') == 'store')) {
                $data = [
                    'title' => translate('messages.order_push_title'),
                    'description' => translate('messages.new_order_push_description'),
                    'order_id' => $order->id,
            ...
            try {
                if ($order->order_status == 'confirmed' && !has_payment_offline($order->payment_method) && config('mail.status')) {
...
//        return $user_name . $user_id;
    ...
    public static function payment_amount($order_payment_methods, $type){
        foreach ($order_payment_methods as $method){
            if($method->payment_methods == $type){
                return $method;
            }
        }
        return null;
    ...
    public static function user_report($type_one, $type){
        foreach ($type_one as $method){
            if($method->transaction_types == $type){
                return $method->total_credit;
            }
        }
        return 0;
    }
