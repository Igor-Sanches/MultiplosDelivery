    ...
    <div class="row">
        @foreach($top_customer as $key=>$item)
            <div class="col-6 col-md-4 mt-2 initial--27"
                 onclick="location.href='{{route('admin.customer.view',['index', $item['user_id']])}}'">
                <div class="grid-card min-height-170">
                    <label class="label_1">Orders : {{$item['count']}}</label>
                    <center class="mt-6">
                        <img class="img--60 img--circle" onerror="this.src='{{my_asset('assets/admin/img/160x160/img1.jpg')}}'" src="{{my_asset('storage/app/public/profile/'.$item->customer->image??'')}}">
                    </center>
