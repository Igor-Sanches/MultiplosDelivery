...
<div class="content container-fluid">
    ...
    <div class="page-header report-page-header">
        <div class="d-flex">
            <img src="{{my_asset('assets/admin/img/store-report.svg')}}" class="page-header-icon" alt="">
    ...
    <div class="store-report-content">
        <div class="left-content">
            <div class="left-content-card">
                <img src="{{my_asset('/assets/admin/img/report/store.svg')}}" alt="">
                <div class="info">
            ...
            <div class="left-content-card">
                <img src="{{my_asset('/assets/admin/img/report/cart.svg')}}" alt="">
                <div class="info">
            ...
            <div class="left-content-card">
                <img src="{{my_asset('/assets/admin/img/report/product.svg')}}" alt="">
                <div class="info">
            ...
            <div class="card h-100 bg-white payment-statistics-shadow">
                ...
                <div class="card-body px-0 pt-0">
                    ...
                    <div class="apex-legends">
                        <div class="before-bg-money">
                            @php($cashPayments = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'cash_on_delivery'))
                            <span>{{ translate('Cash Payments') }}
                                ({{ count($order_payment_methods)>0?\App\CentralLogics\Helpers::number_format_short(isset($cashPayments)?$cashPayments->total_order_amount:0):0 }})</span>
                        </div>
                        <div class="before-bg-card-debit-off">
                            @php($payment_debit_card_offline = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'payment_debit_card_offline'))
                            <span>{{ translate('messages.payment_debit_card_offline') }} (
                                {{ count($order_payment_methods)>0?\App\CentralLogics\Helpers::number_format_short(isset($payment_debit_card_offline)?$payment_debit_card_offline->total_order_amount:0):0 }})</span>
                        </div>
                        <div class="before-bg-card-credit-off">
                            @php($payment_credit_card_offline = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'payment_credit_card_offline'))
                            <span>{{ translate('messages.payment_credit_card_offline') }} (
                                {{ count($order_payment_methods)>0?\App\CentralLogics\Helpers::number_format_short(isset($payment_credit_card_offline)?$payment_credit_card_offline->total_order_amount:0):0 }})</span>
                        </div>
                        <div class="before-bg-pix-off">
                            @php($payment_pix_offline = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'payment_pix_offline'))
                            <span>{{ translate('messages.payment_pix_offline') }} (
                                {{ count($order_payment_methods)>0?\App\CentralLogics\Helpers::number_format_short(isset($payment_pix_offline)?$payment_pix_offline->total_order_amount:0):0 }})</span>
                        </div>
                        <div class="before-bg-wallet">
                            @php($wallet = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'wallet'))
                            <span>{{ translate('messages.Wallet') }}
                                ({{ count($order_payment_methods)>0?\App\CentralLogics\Helpers::number_format_short(isset($wallet)?$wallet->total_order_amount:0):0 }})</span>
                        </div>
                    </div>
                </div>
            </div>
...
@push('script')
    <script src="{{my_asset('assets/admin')}}/vendor/chart.js/dist/Chart.min.js"></script>
    <script src="{{my_asset('assets/admin')}}/vendor/chart.js.extensions/chartjs-extensions.js"></script>
    <script src="{{my_asset('assets/admin')}}/vendor/chartjs-plugin-datalabels/dist/chartjs-plugin-datalabels.min.js"></script>


    <!-- Apex Charts -->
    <script src="{{my_asset('/assets/admin/js/apex-charts/apexcharts.js')}}"></script>
    <!-- Apex Charts -->
@endpush
...
@push('script_2')
    ...
    <script>
        ...
        @php($wallet = \App\CentralLogics\Helpers::payment_amount($order_payment_methods, 'wallet'))
        var options = {
            series: [{{ count($order_payment_methods)>0?isset($cashPayments)?$cashPayments->order_count:0:0 }}, {{ count($order_payment_methods)>0?isset($payment_debit_card_offline)?$payment_debit_card_offline->order_count:0:0 }}, {{ count($order_payment_methods)>0?isset($payment_credit_card_offline)?$payment_credit_card_offline->order_count:0:0 }}, {{ count($order_payment_methods)>0?isset($payment_pix_offline)?$payment_pix_offline->order_count:0:0 }}, {{ count($order_payment_methods)>0?isset($wallet)?$wallet->order_count:0:0 }}],
            chart: {
                width: 320,
                type: 'donut',
            },
            labels: [
                '{{__('messages.payment_cash')}} ({{ count($order_payment_methods)>0?isset($cashPayments)?$cashPayments->total_order_amount:0:0 }})',
                '{{__('messages.payment_debit_card_offline')}} ({{ count($order_payment_methods)>0?isset($payment_debit_card_offline)?$payment_debit_card_offline->total_order_amount:0:0 }})',
                '{{__('messages.payment_credit_card_offline')}} ({{ count($order_payment_methods)>0?isset($payment_credit_card_offline)?$payment_credit_card_offline->total_order_amount:0:0 }})',
                '{{__('messages.payment_pix_offline')}} ({{ count($order_payment_methods)>0?isset($payment_pix_offline)?$payment_pix_offline->total_order_amount:0:0 }})',
                '{{__('messages.wallet')}} ({{ count($order_payment_methods)>0?isset($wallet)?$wallet->total_order_amount:0:0 }})'
            ],
            dataLabels: {
                enabled: false,
                style: {
