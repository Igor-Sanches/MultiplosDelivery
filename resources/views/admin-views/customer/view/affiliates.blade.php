        ...
        <div class="card">
            <div class="card-header border-0 py-2">
                <div class="search--button-wrapper">
                    <h5 class="card-title"> {{translate('affiliates_list')}} <span class="badge badge-soft-secondary">{{ $affiliates->total() }}</span></h5>
            ...
            <div class="table-responsive datatable-custom">
                <table id="columnSearchDatatable"
                    ...
                    <tr>
                        <th class="border-0 pl-4">{{translate('messages.image')}}</th>
                    ...
                    @foreach ($affiliates as $key => $affiliate)
                        <tr class="">
                            <td>
                                <div>
                                    <a href="{{route('admin.customer.view', ['index', $affiliate->id])}}" class="table-rest-info" alt="view store">
                                        <img class="img--60 circle" onerror="this.src='{{my_asset('assets/admin/img/160x160/img1.jpg')}}'"
                                             src="{{my_asset('storage/app/public/profile')}}/{{$affiliate['image']}}">
                                        <div class="info"><div class="text--title">
                                                {{Str::limit($affiliate->f_name.' '.$affiliate->l_name,50,'...')}}
                                            </div>
                                            <div class="font-light">
                                                {{translate('messages.id')}}:{{$affiliate->id}}
                                            </div>
                                        </div>
                                    </a>
                                </div>
                            ...
                            <td>
                                <a class="btn action-btn btn--warning btn-outline-warning"
                                   href="{{ route('admin.customer.view', ['index', $affiliate['id']]) }}"
