            ...
            @if(session()->has('cart') && count( session()->get('cart')) > 0)
                ...
                <tr>
                    <td class="media align-items-center cursor-pointer" onclick="quickViewCartItem({{$cartItem['id']}}, {{$key}})">
                        <img class="avatar avatar-sm mr-1" src="{{my_asset('storage/app/public/product')}}/{{$cartItem['image']}}"
                                onerror="this.src='{{my_asset('assets/admin/img/100x100/2.png')}}'" alt="{{$cartItem['name']}} image">
